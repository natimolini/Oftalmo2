...
def get_ds_pressao(nr_atendimento):
    query = """
    SELECT ds_observacao FROM oft_tonometria WHERE nr_seq_consulta = (SELECT nr_sequencia FROM oft_consulta WHERE nr_atendimento = :nr_atendimento AND ROWNUM = 1)
    """
