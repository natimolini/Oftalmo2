...
def get_ds_anamnese(nr_atendimento):
    query = """
    SELECT ds_anamnese FROM oft_anamnese WHERE nr_seq_consulta = (SELECT nr_sequencia FROM oft_consulta WHERE nr_atendimento = :nr_atendimento AND ROWNUM = 1)
    """
